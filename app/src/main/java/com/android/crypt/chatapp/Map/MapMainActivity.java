...
import com.android.crypt.chatapp.Map.adapter.AddressAdapter;
...
public class MapMainActivity extends BaseActivity implements SensorEventListener{
    ...
    MapView mMapView;
    @BindView(R.id.m_iv_center_location)
    ...
    public void onSensorChanged(SensorEvent event) {
        ...
        switch (event.sensor.getType()) {
            case Sensor.TYPE_ORIENTATION: {
                ...
                x += 0;//getScreenRotationOnPhone(mContext);
                x %= 360.0F;
                if (x > 180.0F)
                    x -= 360.0F;
                else if (x < -180.0F)
...
//                        Toast.makeText(MapMainActivity.this, "搜索", Toast.LENGTH_SHORT).show();
                        startActivityForResult(new Intent(MapMainActivity.this, MapSearchActivity.class), SEARCHREQUESTCODE);
                        break;
                    case R.id.m_iv_location:
//                        Toast.makeText(MapMainActivity.this, "定位", Toast.LENGTH_SHORT).show();
    ...
    private void sendPosition(PoiItem poiItem){
        ...
        model.title = poiItem.getTitle();
        model.detail = poiItem.getSnippet() ;
        model.lat =  poiItem.getLatLonPoint().getLatitude() ;
        model.lon = poiItem.getLatLonPoint().getLongitude();


        Intent resendIntent = new Intent();
        resendIntent.putExtra("position", model);
